...
export async function generateZPayQRCode(orderData: {
  ...
  try {
    // ZPay API接口
    const zpayUrl = 'https://zpayz.cn/mapi.php'
    ...
    // 生成签名
    const merchantKey = import.meta.env.VITE_ZPAY_MERCHANT_KEY || ''
    if (!merchantKey) {
      throw new Error('缺少商户密钥配置 VITE_ZPAY_MERCHANT_KEY')
    }
    
    const signature = await generateMD5Signature(params, merchantKey)
    params.sign = signature
    
    console.log('🔐 ZPay请求参数:', params)
    
    // 发送POST请求到ZPay API
    const response = await fetch(zpayUrl, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`ZPay API请求失败: ${response.status}`)
    }
    ...
    if (result.code !== 1) {
      throw new Error(`ZPay错误: ${result.msg || '支付订单创建失败'}`)
    }
    
    return {
      qr_code: result.qrcode || result.img,
      order_id: orderData.out_trade_no
    }
    
  } catch (error) {
    console.error('生成ZPay二维码失败:', error)
    throw error
  }
}
...
export async function createSubscriptionOrder(subscriptionType: SubscriptionType): Promise<string> {
  try {
    ...
    const ZPAY_RETURN_URL = import.meta.env.VITE_ZPAY_RETURN_URL || `${window.location.origin}/payment-result`

    if (ZPAY_PID && ZPAY_MERCHANT_KEY && ZPAY_NOTIFY_URL) {
      // 🎯 尝试使用前端直接支付（可能受CORS限制）
      console.log('✅ 尝试前端直接支付模式 - 二维码支付')
      
      try {
        const qrResult = await generateZPayQRCode({
          pid: ZPAY_PID,
          type: 'alipay', // 支持 alipay, wxpay 等
          out_trade_no: outTradeNo,
        ...
        console.warn('⚠️ 前端直接支付失败（可能是CORS限制）:', frontendError)
        console.log('🔄 尝试使用后端API...')
        ...
        try {
          ...
          if (result.qr_code) {
            return JSON.stringify({
              ...
              order_id: result.order_id || result.out_trade_no,
              out_trade_no: result.out_trade_no,
              amount: result.amount,
              name: result.subscription_name
            })
          } else {
            throw new Error('后端API未返回二维码')
          }

        } catch (backendError) {
          console.warn('⚠️ 后端API也不可用:', backendError)
          // 最终回退到模拟支付
          throw new Error('支付服务暂时不可用，建议使用后端服务或配置Supabase Edge Function')
        }
      }

    } else {
      // 🔄 回退到后端API模式
      console.log('⚠️ 前端配置不完整，尝试使用后端API')
      ...
      try {
        ...
        // 🎭 最后的回退：模拟支付（仅开发环境）
        console.warn('后端API也不可用，使用模拟支付模式')
        ...
        const mockPaymentUrl = `${window.location.origin}/mock-payment?${mockPaymentParams.toString()}`
        
        // 更新订单的支付URL
        await supabase
          .from('orders')
          .update({ pay_url: mockPaymentUrl })
          .eq('out_trade_no', outTradeNo)

        return JSON.stringify({
          type: 'mock_payment',
          pay_url: mockPaymentUrl,
          order_id: order.id,
          out_trade_no: outTradeNo,
          amount: plan.price,
          name: plan.name
...
export async function getUserMembershipStatus(): Promise<UserMembership | null> {
  try {
    ...
    // 如果没有记录，返回null（表示用户还没有会员记录）
    if (!data || data.length === 0) {
      console.log('用户还没有会员记录，将创建默认的免费会员记录')
      ...
      try {
        const { data: newMembership, error: insertError } = await supabase
          ...
          .insert({
            ...
            trial_audio_plays_remaining: 0
          })
          .select()
          .single()

        if (insertError) {
          console.error('创建默认会员记录失败:', insertError)
          return null
        }

        return newMembership
      } catch (insertErr) {
        console.error('创建默认会员记录异常:', insertErr)
        return null
      }
    }
