...
export async function createSubscriptionOrder(subscriptionType: SubscriptionType): Promise<string> {
  try {
    ...
    // Call backend API to create subscription order
    const response = await fetch('http://localhost:8000/api/create_subscription_order', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        payment_type: 'alipay', // Default to Alipay for QR code
