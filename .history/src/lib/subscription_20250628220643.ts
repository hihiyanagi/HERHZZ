...
export async function generateZPayQRCode(orderData: {
  ...
  name: string
  money: string
}, useProxy: boolean = false): Promise<{
  qr_code: string
  order_id: string
}> {
  try {
    // 根据是否使用代理选择不同的URL
    const zpayUrl = useProxy ? '/api/zpay/mapi.php' : 'https://zpayz.cn/mapi.php'
    ...
    console.log('🌐 使用URL:', zpayUrl, useProxy ? '(代理)' : '(直接)')
  ...
  try {
    ...
    try {
      ...
      console.log('📊 Supabase auth 响应:', {
        user: data.user ? '已登录' : '未登录',
        error: authError
      })
      
      if (authError) {
        console.error('❌ Supabase auth 错误:', authError)
        throw new Error(`认证错误: ${authError.message}`)
      }
      
      if (!data.user) {
        console.error('❌ 用户数据为空')
        throw new Error('用户未登录，请先登录后再进行购买')
      }
      
    } catch (authCheckError) {
      console.error('❌ 检查用户状态失败:', authCheckError)
      throw new Error(`无法验证用户状态: ${authCheckError.message}`)
    }
    ...
    try {
      ...
      console.warn('⚠️ 后端API不可用:', backendError)
      
      // 如果后端不可用，提供清晰的解决方案指导
      const errorMessage = `
支付服务配置问题：

1. 🚨 前端无法直接调用ZPay API（CORS限制）
2. 🔧 后端服务未启动或配置错误

💡 解决方案：
• 启动后端服务：cd backend && python main.py
• 或配置Supabase Edge Function
• 或使用模拟支付测试功能

详情请查看 FRONTEND_SUBSCRIPTION_SETUP.md
      `
