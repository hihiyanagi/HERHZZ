...
export async function generateZPayQRCode(orderData: {
  ...
  console.log('🔐 二维码支付请求参数:', fullParams)

  try {
    // 调用ZPay API获取二维码
    const response = await fetch('https://zpayz.cn/api/qr.php', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
      body: new URLSearchParams(fullParams as any)
    })
    ...
    if (result.code !== 1) {
      throw new Error(`ZPay错误: ${result.msg || '未知错误'}`)
    }

    return {
      qr_code: result.qr_code,
      order_id: result.order_id || orderData.out_trade_no
    }
  } catch (error) {
    console.error('获取二维码失败:', error)
    throw new Error('获取支付二维码失败，请稍后重试')
  }
}
...
export async function createSubscriptionOrder(subscriptionType: SubscriptionType): Promise<string> {
  try {
    ...
    // 验证必需的ZPay配置（二维码支付不需要回调URL）
    if (!ZPAY_PID) {
      throw new Error('ZPay配置不完整，请检查环境变量：VITE_ZPAY_PID')
    }

    console.log('🚀 使用ZPay二维码支付，准备生成二维码...')
    
    // 使用ZPay配置生成二维码
    const qrData = await generateZPayQRCode({
      pid: ZPAY_PID,
      type: 'alipay', // 支付宝二维码支付
      out_trade_no: outTradeNo,
      notify_url: ZPAY_NOTIFY_URL, // 可选：如果需要回调通知
      name: plan.name,
    ...
    await supabase
      ...
      .update({ 
        ...
        pay_url: qrData.qr_code // 也存储在pay_url字段中，方便兼容
      })
