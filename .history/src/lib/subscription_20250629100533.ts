...
export async function createSubscriptionOrder(subscriptionType: SubscriptionType): Promise<string> {
  try {
    ...
    // Get user session for auth header with refresh
    let { data: { session } } = await supabase.auth.getSession()
    
    // 如果session不存在或token即将过期（5分钟内），尝试刷新
    if (!session?.access_token) {
      console.log('🔄 Session不存在，尝试刷新token...')
      ...
      try {
        ...
        console.warn('⚠️ 无法解析token过期时间，使用现有token')
      }
    }

    // No conversion needed - backend now uses same values as frontend
    ...
    console.log('✅ 后端QR码API响应:', result)

    // Validate response structure
    if (!result.out_trade_no) {
      throw new Error('后端响应缺少订单号')
    }

    // 只支持二维码支付
    if (result.qr_code) {
      console.log('📱 成功获取二维码链接')
      ...
      throw new Error('未获取到二维码，请检查支付配置')
    }
