...
export default function SubscriptionPage() {
  ...
  const handleCreateSubscriptionOrder = async () => {
    ...
    try {
      ...
      const paymentResponse = await createSubscriptionOrder(selectedPlan.type)
      ...
      if (paymentResponse) {
        try {
          ...
          // 🎯 处理不同类型的支付响应
          if (paymentData.type === 'jump_url') {
            // 跳转支付模式
            toast({
              title: "跳转到支付页面",
              description: "正在为您跳转到ZPay支付页面..."
            })
          ...
          } else if (paymentData.type === 'qr_code') {
            // 二维码支付模式
            toast({
              title: "订单创建成功！",
            ...
            const qrCodeImage = await generateQRCodeImage(paymentData.qr_code)
            if (qrCodeImage) {
          ...
          } else if (paymentData.type === 'mock_payment') {
            // 模拟支付模式
            toast({
              title: "进入模拟支付模式",
              description: "正在跳转到模拟支付页面..."
            })
          ...
          console.error('解析支付响应失败:', parseError)
          toast({
            title: "支付响应解析失败",
