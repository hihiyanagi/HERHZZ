...
const Introduction = ({ onContinue }: IntroductionProps) => {
  ...
  useEffect(() => {
    ...
    return () => {
      paragraphs?.forEach(p => observer.unsubscribe(p));
    };
  }, []);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-center p-6 relative">
      ...
      <h3 className="text-3xl md:text-4xl font-bold text-white mb-12 z-10 text-center animate-pulse hover:animate-bounce transition-all duration-500 tracking-wide">
        女性。睡眠。月亮。
      </h3>
