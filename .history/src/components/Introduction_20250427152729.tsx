...
const Introduction = ({ onContinue }: IntroductionProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-center p-6 relative bg-gradient-to-b from-starry-dark to-black">
      ...
      <h3 className="text-xl md:text-2xl font-semibold text-white mb-12 z-10">
        女性。睡眠。月亮。
      </h3>
      
      <div ref={textRef} className="max-w-2xl text-white mb-10 z-10">
        <div className="fade-in-section space-y-8 p-8 rounded-lg backdrop-blur-sm leading-loose">
      ...
      {onContinue && (
        <button 
          onClick={onContinue}
          className="px-8 py-3 bg-transparent hover:bg-white/20 border border-white text-white rounded-full font-medium transition-all duration-300 transform hover:scale-105 hover:shadow-lg z-10"
        >
          继续
        </button>
      )}
