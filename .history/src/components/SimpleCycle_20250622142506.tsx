        ...
        if (user && !useLocalStorage) {
          try {
            ...
            const result = await addCycle(
              formattedDate, 
              [], 
              `周期开始于 ${format(date, "yyyy年MM月dd日")}`
            );
            console.log('✅ 数据保存到Supabase成功:', result);
            setHasSubmitted(true);
            return;
          } catch (supabaseError) {
            console.error('❌ Supabase保存失败，尝试本地存储:', supabaseError);
            ...
            <div className="backdrop-blur-sm bg-white/5 rounded-lg p-6 mb-6 animate-fade-in">
              ...
              <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                <p className="text-blue-200 text-xs">
                  💡 {user ? (useLocalStorage ? '使用本地存储备份' : '数据将同步到云端') : '数据将保存到本地存储'}
                </p>
              </div>
            </div>
          </div>
