...
const CycleInfo = () => {
    ...
    return (
      <div className="text-center mb-6">
        {isVipUser ? (
          ...
          <div className="inline-flex items-center px-4 py-2 rounded-full bg-white/10 border border-white/20 text-white/80">
            <span className="text-sm">免费用户 · 每个周期可试听1首音频</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6">
            ...
            >
              ...
              {index === 0 && (
                <div className="absolute top-3 left-3 z-10">
                  <div className="px-2 py-1 rounded-full bg-green-500/20 border border-green-400/30 text-green-300 text-xs font-medium">
                    免费
                  </div>
                </div>
              )}
      ...
      {/* 升级提示卡片 */}
      {!isVipUser && !membershipLoading && (
        <div className="mt-12 p-6 rounded-lg bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-400/20">
          <div className="text-center">
            ...
            <p className="text-white/70 mb-4">
              升级为HERHZZZ会员，畅享12种高品质助眠音频，专为你的生理周期定制
            </p>
            <button
              onClick={() => navigate('/subscription')}
              className="px-6 py-3 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 text-white font-medium hover:from-purple-600 hover:to-pink-600 transition-all shadow-lg"
            >
              查看会员套餐
            </button>
          </div>
