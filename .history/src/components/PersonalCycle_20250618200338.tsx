...
const phaseGuides: Record<CyclePhase, PhaseGuideText> = {
  ...
  follicular: {
    ...
    sounds: [
      ...
      { name: "🎵 银光森林", audioSrc: "/audio/yinanguang_senlin.mp3" },
      { name: "🎵 粉樱温泉", audioSrc: "/audio/fensai_wenquan.mp3" }
    ],
    guideText: [
  ...
  ovulation: {
    ...
    sounds: [
      ...
      { name: "🎵 雪地猫步", audioSrc: "/audio/xuedi_maobu.mp3" },
      { name: "🎵 秋岛之夜", audioSrc: "/audio/qiu_dao_yezhi.mp3" }
    ],
    guideText: [
      "有点清醒，也有点敏感。",
      "今天的你，像月夜盛开的花，",
  ...
  luteal: {
    phase: "luteal",
    sounds: [
      { name: "🎵 月云软语", audioSrc: "/audio/yueyun_ruanyu.mp3" },
      { name: "🎵 苔藓蒸汽", audioSrc: "/audio/taixian_qizhang.mp3" },
      { name: "🎵 夜空寺庙", audioSrc: "/audio/ye_kong_sitao.mp3" }
    ],
    guideText: [
                ...
                <div className="backdrop-blur-md bg-white/5 rounded-lg p-6 mb-6 animate-fade-in" style={{ animationDelay: '0.3s' }}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    {phaseGuides[currentPhase].sounds.map((sound, index) => (
                      <AudioPlayer key={index} title={sound.name} audioSrc={sound.audioSrc} />
                    ))}
                  </div>
