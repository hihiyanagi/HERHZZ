...
const Moon: React.FC<MoonProps> = ({ size = 200, moonColorScheme = "cool" }) => {
  ...
  const glowColors = moonColorScheme === "warm"
    ? { from: "from-[#f5e8a5]/20", to: "to-[#e6c86e]/40" } // 温暖的月光光晕
    ...
    <div className="relative flex items-center justify-center animate-float">
      ...
      >
        ...
        <div className="absolute inset-0 opacity-30 mix-blend-overlay">
          {moonColorScheme === "warm" && (
            <>
              <div className="absolute w-12 h-12 rounded-full bg-[#4d402e] top-[20%] left-[30%] opacity-40"></div>
              <div className="absolute w-8 h-8 rounded-full bg-[#3d3424] bottom-[25%] right-[35%] opacity-30"></div>
            </>
          )}
        </div>
      </div>
