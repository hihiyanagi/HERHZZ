...
def generate_zpay_jump_url(
    ...
    Args:
        base_url: ZPay 基础URL（如 https://zpayz.cn/submit.php）
        params: 支付参数字典
        merchant_key: 商户密钥
    ...
    """
    # 生成签名
    sign = generate_md5_signature(params, merchant_key)
    
    # 添加签名到参数中
    params_with_sign = params.copy()
    params_with_sign['sign'] = sign
    params_with_sign['sign_type'] = 'MD5'
    
    # 使用 urllib.parse.urlencode 生成查询字符串
    query_string = urlencode(params_with_sign)
    
    # 拼接完整的跳转链接
    jump_url = f"{base_url}?{query_string}"
...
def normalize_payment_type(payment_type: str) -> str:
    ...
    Args:
        payment_type: 支付方式（alipay, wxpay等）
    ...
    type_mapping = {
        'alipay': 'alipay',
        'wxpay': 'wxpay',
        'wechat': 'wxpay',  # 兼容不同的命名
        'union': 'union'
    }
    
    return type_mapping.get(payment_type.lower(), payment_type) 