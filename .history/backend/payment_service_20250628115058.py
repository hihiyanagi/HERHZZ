...
from utils import generate_md5_signature, generate_zpay_jump_url, normalize_payment_type
...
class PaymentService:
    ...
    def __init__(self):
        ...
        self.zpay_jump_url = "https://zpayz.cn/submit.php"  # 页面跳转支付URL
        ...
        self.return_url = os.getenv("ZPAY_RETURN_URL", "")  # 支付成功后跳转页面
        
        if not self.merchant_id or not self.merchant_key:
            raise ValueError("缺少 ZPay 配置信息，请检查环境变量")
    
    def generate_subscription_payment_jump_url(
        self,
        request: CreateSubscriptionOrderRequest,
        out_trade_no: str,
        custom_return_url: Optional[str] = None
    ) -> str:
        """
        生成订阅支付的 ZPay 页面跳转链接
        ...
        Args:
            request: 订阅支付请求数据
            out_trade_no: 商户订单号
            custom_return_url: 自定义返回URL（可选）
            
        Returns:
            str: 支付跳转链接
        """
        # 标准化支付方式
        payment_type = normalize_payment_type(request.payment_type)
        
        # 确定返回URL
        return_url = custom_return_url or request.return_url or self.return_url
        ...
        params = {
            ...
            "money": f"{request.subscription_amount:.2f}",  # 确保两位小数
            "notify_url": self.notify_url,
            "param": f"subscription_type:{request.subscription_type.value}"  # 附加订阅类型信息
        }
        ...
        # 生成跳转链接
        jump_url = generate_zpay_jump_url(
            base_url=self.zpay_jump_url,
            params=params,
    ...
    def generate_payment_jump_url(
        ...
        params = {
            ...
            "out_trade_no": out_trade_no,
            "name": request.name,
            "money": f"{request.amount:.2f}",  # 确保两位小数
            "notify_url": self.notify_url,
            "param": ""  # 附加参数，可以传递用户ID等信息
        }
